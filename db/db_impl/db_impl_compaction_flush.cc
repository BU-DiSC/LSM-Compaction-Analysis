...
void DBImpl::NotifyOnFlushBegin(ColumnFamilyData* cfd, FileMetaData* file_meta,
  ...
  mutex_.Unlock();
  {
    ...
    // !YBS-aug17-XX
    info.file_timestamp = file_meta->fd.GetFileTimestamp();
    info.expired_ttl = file_meta->fd.GetExpiredTTL();
    // !END
  ...
  if (to_level != level) {
    ...
    for (const auto& f : vstorage->LevelFiles(level)) {
      ...
      edit.AddFile(to_level, f->fd.GetNumber(), f->fd.GetPathId(),
                   ...
                   f->fd.GetFileTimestamp(), // !YBS-aug17-XX!
                   f->fd.GetExpiredTTL(), // !YBS-aug17-XX!
