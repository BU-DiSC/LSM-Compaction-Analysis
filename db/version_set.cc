...
// !YBS-aug17-XX
#include <iostream> 
#include "examples/__working_branch/emu_environment.h" 
#include "examples/__working_branch/stats.h" 
#include <fstream> 
#include <chrono> 
#include <ctime>  
#include <sstream> 
  ...
  for (int level = 0; level < cfd_->NumberLevels(); level++) {
    ...
    for (const auto& file : vstorage->LevelFiles(level)) {
      ...
      files.emplace_back(SstFileMetaData{
          ...
          file->fd.GetFileTimestamp(), // !YBS-aug17-XX!
          file->fd.GetExpiredTTL(), // !YBS-aug17-XX!
...
void Version::PrepareApply(
  ...
  storage_info_.PrintInfo(); // !YBS-aug17-XX!
}
...
void VersionStorageInfo::ComputeCompactionScore(
  ...
  EmuEnv* _env = EmuEnv::getInstance(); // !YBS-aug17-XX!
  for (int level = 0; level <= MaxInputLevel(); level++) {
    ...
    if (level == 0) {
      ...
      // !YBS-aug17-XX
      int l0_dpl_th_expired_count = 0;
      int l0_expired_ttl_weight = 100;
      std::chrono::time_point<std::chrono::system_clock> current_timestamp = std::chrono::system_clock::now();
      // auto c = (std::chrono::duration<double, std::milli>(current_timestamp - temp[i].file->fd.file_timestamp)).count();
      // !END
      ...
      for (auto* f : files_[level]) {
        if (!f->being_compacted) {
          ...
          num_sorted_runs++;
          auto c = (std::chrono::duration<double, std::milli>(current_timestamp - f->fd.file_timestamp)).count();
          double level_delete_persistence_latency = _env->GetLevelDeletePersistenceLatency(level, _env);
          if( c > level_delete_persistence_latency*1000 && level_delete_persistence_latency != -1 && f->num_deletions > 0) {
            if (_env->verbosity >=2)
              std::cout << c << " > " << level_delete_persistence_latency << std::endl;
            // std::cout << " kjhajkdhgakjghajdhadsjghajdkghakjlghajfdghjkfadhgkjdfhgjkahdgkljahgjkdfhgadfhgkjahdfgkjahghadskjghajg " << std::endl;
            ++l0_dpl_th_expired_count;
            f->fd.expired_ttl = true;
          }
        }
      }
      ...
      if (compaction_style_ == kCompactionStyleFIFO) {
        ...
        if(_env->compaction_pri == 5)
          score = static_cast<double>(num_sorted_runs) / mutable_cf_options.level0_file_num_compaction_trigger + l0_dpl_th_expired_count * l0_expired_ttl_weight;
        else
        ...
        if (compaction_style_ == kCompactionStyleLevel && num_levels() > 1) {
          ...
          if (score > 1 && _env->verbosity >= 2)
            std::cout << "for level = 0: score > 1 : score = " << score << std::endl;
        }
      }
    } 
    // for all level-i with i>0
    else {
      // Compute the ratio of current size to size limit.
      uint64_t level_bytes_no_compacting = 0;
      int level_dpl_th_expired_count = 0;
      int expired_ttl_weight = 100;
      std::chrono::time_point<std::chrono::system_clock> current_timestamp = std::chrono::system_clock::now();
      ...
      for (auto f : files_[level]) {
        if (!f->being_compacted) {
          ...
          if( c > level_delete_persistence_latency*1000 && level_delete_persistence_latency != -1 && f->num_deletions > 0) {
            if (_env->verbosity >=2)
              std::cout << c << " > " << level_delete_persistence_latency << std::endl;
              // std::cout << " 90348578934659026598743659346029436240398652437895624378956438756234985623485026345823475623452 " << std::endl;
            ++level_dpl_th_expired_count;
            f->fd.expired_ttl = true;
          }
        }
      }
      if(_env->compaction_pri == 5)
        score = static_cast<double>(level_bytes_no_compacting) / MaxBytesForLevel(level) + level_dpl_th_expired_count * expired_ttl_weight;
      else 
        score = static_cast<double>(level_bytes_no_compacting) / MaxBytesForLevel(level);

      if (score > 1 && _env->verbosity >=2)
        std::cout << "for level > 1: score > 1 : score = " << score << std::endl;
    }
  ...
  //   std::cout << compaction_level_[j] << "\t";
  // std::cout << "\n Score: ";
  // for (int j = 0; j < num_levels() - 1; j++) 
  //   std::cout << compaction_score_[j] << "\t";
  // std::cout << "\n";
  ...
  // std::cout << "\n";
  if (_env->verbosity >= 2)
    std::cout << "chosen level = " << compaction_level_[0] << " :: score = " << compaction_score_[0] << std::endl;
  ...
  // !YBS-sep08=XX
  // Full level compaction code
  Stats* fade_stats = Stats::getInstance();
  // std::cout << "Compaction level = " << compaction_level_[0] << std::endl;
  if (_env->compaction_pri == 8 && (_env->current_op != 'Q' && _env->current_op != 'S' && _env->current_op != ' ') ) { // !YBS-sep18-XX!
    // std::cout << "DEBUG : score = " << compaction_score_[0] << " :: INS = " << _env->current_op << std::endl;
    for (auto f : files_[compaction_level_[0]]) {
      f->marked_for_compaction = true;
    }
    if (fade_stats->levels_in_tree > 0) {
      for (int i = 0; i < fade_stats->levels_in_tree - 1; i++) {
        if (fade_stats->files_in_level[i] == 1) {
          for (auto f : files_[i]) {
  ...
  for (int level = 0; level < num_levels() - 1; level++) {
    ...
    switch (compaction_pri) {
      case kByCompensatedSize:
        ...
        if(!_env->version_set_FEF) std::cout << "Data movement policy: kByCompensatedSize" << std::endl; // !YBS-sep06-XX!
        break;
      case kOldestLargestSeqFirst:
        ...
        if(!_env->version_set_FEF) std::cout << "Data movement policy: kOldestLargestSeqFirst" << std::endl; // !YBS-sep06-XX!
        break;
      case kOldestSmallestSeqFirst:
        ...
        if(!_env->version_set_FEF) std::cout << "Data movement policy: kOldestSmallestSeqFirst" << std::endl; // !YBS-sep06-XX!
        break;
      case kMinOverlappingRatio:
        SortFileByOverlappingRatio(*internal_comparator_, files_[level],
                                   files_[level + 1], &temp);
        if(!_env->version_set_FEF) std::cout << "Data movement policy: kMinOverlappingRatio" << std::endl; // !YBS-sep06-XX!
        break;
      // !YBS-aug17-XX
      case kFADE:
      if(!_env->version_set_FEF) std::cout << "Data movement policy: kFADE" << std::endl; // !YBS-sep06-XX!
        if(_env->verbosity >= 1 && level <= fade_stats->levels_in_tree){
          std::cout << "\nlevel = " << level << std::endl ;
          std::cout << "BEFORE SORTING :: ";
          for (long i=0; i<(long)temp.size(); ++i)
            std::cout << temp[i].file->fd.packed_number_and_path_id << " (" << temp[i].file->num_deletions << ") [" 
                    << temp[i].file->fd.expired_ttl << "] " ;
          std::cout << std::endl;
        }

        // temp[0].file->fd.file_timestamp
        std::sort(temp.begin(), temp.end(),
                  [&current_timestamp, &level_delete_persistence_latency](const Fsize& f1, const Fsize& f2) -> bool {
                    // std::cout << f1.file->num_deletions << " " << f2.file->num_deletions << "\t[ " << f1.file->fd.packed_number_and_path_id << " " << f2.file->fd.packed_number_and_path_id << " ]" << "\t";
                    // std::cout << "\nfile_id 1 = " << f1.file->fd.packed_number_and_path_id << " num_deletions = " << f1.file->num_deletions << std::endl;
                    // std::cout << "file_id 2 = " << f2.file->fd.packed_number_and_path_id << " num_deletions = " << f2.file->num_deletions << std::endl;
                    ...
                    if( c1 > level_delete_persistence_latency*1000 && c2 > level_delete_persistence_latency*1000 && level_delete_persistence_latency != -1 ) {
                      if (c1 == c2) {
                        // std::cout << "BOTH EXPIRED: same file-ages: " << f1.file->fd.packed_number_and_path_id << " and " 
                        //         << f2.file->fd.packed_number_and_path_id << " :: checking num_deletes ... -> returning " 
                        //         << (f1.file->num_deletions > f2.file->num_deletions) << "\n";
                        return f1.file->num_deletions > f2.file->num_deletions;
                      }
                      else {
                        // std::cout << "BOTH EXPIRED: different file-ages: " << f1.file->fd.packed_number_and_path_id << " and " 
                        //         << f2.file->fd.packed_number_and_path_id << " -> returning " 
                        //         << ( (c1 > c2) ? f1.file->num_deletions : f2.file->num_deletions ) << "\n";
                        return c1 > c2;
                      }
                    }
                      ...
                      // std::cout << f1.file->fd.packed_number_and_path_id << " EXPIRED -> returning " << f1.file->num_deletions << "\n";
                      return c1 > c2;
                    }
                    else if( c1 <= level_delete_persistence_latency*1000 && c2 > level_delete_persistence_latency*1000 && level_delete_persistence_latency != -1 ) {
                      // std::cout << f2.file->fd.packed_number_and_path_id << " EXPIRED -> returning " << f2.file->num_deletions << "\n";
                      return c2 > c1;
                    }
                    else {
                      // std::cout << "NONE EXPIRED -> returning " << (f1.file->num_deletions < f2.file->num_deletions) << "\n";
                      return f1.file->num_deletions < f2.file->num_deletions;
                    }
                    // if (!1 && c1==c2 ) std::cout << level_delete_persistence_latency;
                    // return f1.file->num_deletions > f2.file->num_deletions;
                  });
      ...
      case kRoundRobin:
      {
        if(!_env->version_set_FEF) std::cout << "Data movement policy: kRoundRobin" << std::endl; 
        int level_next_RR_index = -1;
        if (_env->RR_level_last_file_selected[level] != -1) {
          level_next_RR_index = _env->RR_level_last_file_selected[level] < (int)temp.size() ? _env->RR_level_last_file_selected[level] : 0 ;
          _env->RR_level_last_file_selected[level] = level_next_RR_index;
        }
        if (temp.size() > 0) {
          for (int i = 0; i < level_next_RR_index + 1; i++) {
            // std::cout << "HERE: level_next_RR_index = " << level_next_RR_index << std::endl; 
            // std::cout << "HERE: temp.size() = " << temp.size() << std::endl; 
            Fsize first_temp;
            first_temp.index = temp[0].index;
            first_temp.file = temp[0].file;
            for (int j = 0; j < (int)temp.size() - 1; j++) {
              temp[j].index = temp[j+1].index;
              temp[j].file = temp[j+1].file;
            }
            temp[temp.size()-1].index = first_temp.index;
            temp[temp.size()-1].file = first_temp.file;
          }
          if (_env->verbosity > 3) _env->PrintRRIndices(_env);
          _env->RR_level_last_file_selected[level]++;
        }
        break;
      }
      // !END
      // !YBS-sep07-XX
      case kMinOverlappingGrandparent:
      { 
        if(!_env->version_set_FEF) std::cout << "Data movement policy: kMinOverlappingGrandparent" << std::endl; 
        if (temp.size() > 0) {
          if (level < _env->live_levels - 2) {
            // std::cout << " IF DEBUG MSG: Level = " << level << " (< " << _env->live_levels << " - 2 )" << std::endl;
            SortFileByOverlappingRatio(*internal_comparator_, files_[level],
                                        files_[level + 2], &temp);
          }
          else {
            // std::cout << " else debug msg: Level = " << level << " (>= " << _env->live_levels << " - 2 )" << std::endl;
            SortFileByOverlappingRatio(*internal_comparator_, files_[level],
                                      files_[level + 1], &temp);
          }
        }
      ...
      case kFullLevel:
        SortFileByOverlappingRatio(*internal_comparator_, files_[level],
                                    files_[level + 1], &temp);
        if(!_env->version_set_FEF) std::cout << "Data movement policy: kFullLevel" << std::endl; // !YBS-sep06-XX!
        break;
      // !END
      default:
        if(!_env->version_set_FEF) std::cout << "Data movement policy: default" << std::endl; // !YBS-sep06-XX!
        assert(false);
    }
    _env->version_set_FEF = false ? true : true; // !YBS-sep06-XX!
    // if(!_env->version_set_FEF)  std::cout << "Setting version_set_FEF to: " << _env->version_set_FEF << std::endl; // !YBS-sep06-XX!
    
    assert(temp.size() == files.size());

    // initialize files_by_compaction_pri
...
// !YBS-aug17-XX
void VersionStorageInfo::PrintInfo() {

  EmuEnv* _env = EmuEnv::getInstance();
  Stats* fade_stats = Stats::getInstance();
      
  ofstream file_age_file;
  ...
  assert(file_age_file);
  file_age_file << "file_id" << "\t" << "level" << "\t" << "pt_ts_count" << "\t" << "file_age" << "\t" 
                << "ttl_exp" << "\t" << "d_th_level" << std::endl; 

  long deletion_count = 0;
  long entry_count = 0;
  std::chrono::time_point<std::chrono::system_clock> current_timestamp = std::chrono::system_clock::now();
  if (!fade_stats->completion_status) {
    fade_stats->files_in_tree = 0;
    fade_stats->files_in_level.resize(0);
    fade_stats->fade_file_info.resize(0);
  }
  
  if(_env->verbosity == 4) {
    std::cout << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx " << std::endl ;
    std::cout << "xxxxxxxxxxxxx PRINTING INFO :: Current timestamp = " << std::chrono::system_clock::to_time_t(current_timestamp) << "xxxxxxxxxxxxx" << std::endl;
    std::cout << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx " << std::endl ;
  }

  for (int level = 0; level < fade_stats->levels_in_tree; level++) { // alternatively have the for-condition as level < num_levels()
    const std::vector<FileMetaData*>& files = files_[level];
    if (!fade_stats->completion_status) {
      fade_stats->files_in_level.push_back(0);
    }

    // populate a temp vector for sorting based on size
    std::vector<Fsize> temp(files.size());
    for (size_t i = 0; i < files.size(); i++) {
      if ( EmuEnv::CheckingVector(files[i]->fd.packed_number_and_path_id ) == 2){
        // std::cout << "MATCH FOUND again file_id = " << files[i]->fd.packed_number_and_path_id << std::endl;
        files[i]->fd.file_timestamp = EmuEnv::GetDumpedDeleteFileTimestamp();
      }
      if (level == fade_stats->levels_in_tree -1) {
        if (_env->verbosity >= 3)
          std::cout << "Re-setting timestamp for file_id = " << files[i]->fd.packed_number_and_path_id << " in level = " << level << "\t";
        
        files[i]->fd.file_timestamp = std::chrono::system_clock::now();
      }
      temp[i].index = i;
      temp[i].file = files[i]; 
      ...
      if (!fade_stats->completion_status) {
        ...
        FadeFileInfo ffi;
        std::ostringstream oss; oss << files[i]->fd.packed_number_and_path_id;
        ffi.file_id = oss.str();
        ffi.file_level = level;
        ffi.file_entry_count = files[i]->num_entries;
        ffi.file_tombstone_count = files[i]->num_deletions;
        ffi.file_size = files[i]->fd.GetFileSize();
        ffi.file_age = (std::chrono::duration<double, std::milli>(current_timestamp - files[i]->fd.file_timestamp)).count();
        ffi.file_ttl_expired = files[i]->fd.GetExpiredTTL();

        fade_stats->fade_file_info.push_back(ffi);
        fade_stats->key_size = files[i]->raw_key_size;
        fade_stats->value_size = files[i]->raw_value_size;

        file_age_file << temp[i].file->fd.packed_number_and_path_id << "\t" << level << "\t" << temp[i].file->num_deletions << "\t"
                << ( (std::chrono::duration<double, std::milli>(current_timestamp - temp[i].file->fd.file_timestamp)).count() > 0 ?
                        (std::chrono::duration<double, std::milli>(current_timestamp - temp[i].file->fd.file_timestamp)).count()/1000 : 0 ) << "\t" 
                << temp[i].file->fd.GetExpiredTTL() << "\t" << _env->GetLevelDeletePersistenceLatency(level, _env) << std::endl;
      }
    }
    ...
    for (long i=0; i<(long)temp.size(); ++i){
      if(_env->verbosity == 4)
        std::cout << temp[i].file->fd.packed_number_and_path_id << " " ;
      if(_env->verbosity == 4){
        std::cout << "(" << temp[i].file->num_deletions << ") ";
        std::time_t now_c = temp[i].file->fd.expired_ttl;
        std::cout << "{" << now_c << "} ";
        auto c = (std::chrono::duration<double, std::milli>(current_timestamp - temp[i].file->fd.file_timestamp)).count();
        std::cout << "[" << c << "] "; 
      }
      deletion_count += temp[i].file->num_deletions;
      entry_count += temp[i].file->num_entries;

      if (!fade_stats->completion_status) {
        fade_stats->files_in_tree++;
        fade_stats->files_in_level[level]++;
      }
  ...
  if (_env->verbosity > 1) {
    std::cout << "#Levels: " << fade_stats->levels_in_tree << " :: [";
    for (int i = 0; i < fade_stats->levels_in_tree; ++i)
      std::cout << fade_stats->files_in_level[i] << " ";
    std::cout << "\b]";
    if (_env->show_progress) 
      std::cout << "                                                                                     " << std::endl;
  }
  ...
  if (!fade_stats->completion_status) {
    fade_stats->key_values_in_tree = entry_count - deletion_count; // NOTE: entry_count = key_value_count + deletion_count 
    fade_stats->point_tombstones_in_tree = deletion_count;
    // !YBS-sep06-XX!
    if (fade_stats->levels_in_tree != num_non_empty_levels_) {
      fade_stats->levels_in_tree = num_non_empty_levels_;
      if (_env->verbosity > 1) {
        if (_env->show_progress) {
          for (int i = 0; i<104; i++){
            std::cout << "\b";
        ...
        std::cout << "New tree-level added -- total levels = " << num_non_empty_levels_;
      }
      _env->AddNewLevel(num_non_empty_levels_, _env);
    }
    // !END
  }
  else if (fade_stats->completion_status && fade_stats->db_open == false)
  {
    for (int level = 0; level < num_levels(); level++) {
      const std::vector<FileMetaData*>& files = files_[level];
      std::vector<Fsize> temp(files.size());
      ...
      if (level == 0 && (long)temp.size() > 0){
        // for (long i=0; i<(long)temp.size(); ++i){
          fade_stats->key_values_in_tree += (temp[0].file->num_entries - temp[0].file->num_deletions); // NOTE: entry_count = key_value_count + deletion_count 
          fade_stats->point_tombstones_in_tree += temp[0].file->num_deletions;
        // }
      }
    } 
  } 
...
Status VersionSet::ProcessManifestWrites(
  ...
  IOStatus io_s;
  {
    ...
    if (s.ok()) {
      if (!first_writer.edit_list.front()->IsColumnFamilyManipulation()) {
        for (int i = 0; i < static_cast<int>(versions.size()); ++i) {
          // std::cout << "11111111111111111111111111111111111111111111111111111111111111111111111" << std::endl;
      ...
      for (int level = 0; level < cfd->NumberLevels(); level++) {
        for (const auto& f :
          ...
          edit.AddFile(level, f->fd.GetNumber(), f->fd.GetPathId(),
                       ...
                       f->fd.GetFileTimestamp(), f->fd.GetExpiredTTL(), // !YBS-aug17-XX!
  ...
  for (auto cfd : *column_family_set_) {
    ...
    for (int level = 0; level < cfd->NumberLevels(); level++) {
      for (const auto& file :
        ...
        // !YBS-aug17-XX
        filemetadata.file_timestamp = file->fd.GetFileTimestamp();
        filemetadata.expired_ttl = file->fd.GetExpiredTTL();
        // !END
