...
bool Compaction::IsTrivialMove() const {
  // std::cout<< "ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ" << std::endl;
...
void Compaction::AddInputDeletions(VersionEdit* out_edit) {
  ...
  for (size_t which = 0; which < num_input_levels(); which++) {
    for (size_t i = 0; i < inputs_[which].size(); i++) {
      ...
      if (_env->verbosity == 4)
        std::cout << "In AddInputDeletions() :: deleting file no.: " << inputs_[which][i]->fd.packed_number_and_path_id << std::endl;
      EmuEnv::DumpDeleteFileTimestamp(inputs_[which][i]->fd.file_timestamp, inputs_[which][i]->fd.packed_number_and_path_id, _env);
      fade_stats->files_read_for_compaction++;
      fade_stats->bytes_read_for_compaction += inputs_[which][i]->fd.file_size;
    }
  }
  fade_stats->compaction_count++;
}
// !END
