...
CompactionJob::CompactionJob(
  ...
  // std::cout<< "YYYYYYYYYYYYYYYYYYYYYYYYYYYY" << std::endl;
...
Status CompactionJob::InstallCompactionResults(
  ...
  EmuEnv* _env = EmuEnv::getInstance(); // !YBS-sep06-XX!
  Stats* fade_stats = Stats::getInstance(); // !YBS-sep06-XX!
  for (auto& sub_compact : compact_->sub_compact_states) {
    for (auto& out : sub_compact.outputs) {
      compaction->edit()->AddFile(compaction->output_level(), out.meta);
      // !YBS-sep06-XX
      std::chrono::time_point<std::chrono::system_clock> returned_timestamp = EmuEnv::GetDumpedDeleteFileTimestamp();
      if (_env->verbosity >= 3) {
        std::cout << "returned timestamp: " << std::chrono::system_clock::to_time_t(returned_timestamp) << " || ";
        std::cout << "In InstallCompactionResults() :: adding file no.: " << out.meta.fd.packed_number_and_path_id << " || ";
      }
      if(out.meta.num_deletions == 0)
        out.meta.fd.file_timestamp = EmuEnv::GetDumpedDeleteFileTimestamp();
        // out.meta.fd.file_timestamp = returned_timestamp;
      if (_env->verbosity == 4)
        std::cout << "checking assignment correctness: " << std::chrono::system_clock::to_time_t(out.meta.fd.file_timestamp)
                << " of file " << out.meta.fd.packed_number_and_path_id << std::endl;
      fade_stats->files_written_after_compaction++;
      fade_stats->bytes_written_after_compaction += out.meta.fd.GetFileSize();
      // std::cout << "writing file id: " << out.meta.fd.packed_number_and_path_id << std::endl;
    }
  }
  // std::cout << "SLEEPING" << std::endl;
  // usleep(1000000);
  // std::cout << compact_->sub_compact_states[0].outputs[0].meta.fd.packed_number_and_path_id << "|||||" << std::chrono::system_clock::to_time_t(compact_->sub_compact_states[0].outputs[0].meta.fd.file_timestamp) << std::endl;
  // std::cout << "compaction_job.cc (1408) :: InstallCompactionResults calling LogAndApply !!" << std::endl;
  // !END
