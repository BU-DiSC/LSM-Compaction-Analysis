...
struct FileDescriptor {
  ...
  // !YBS-aug17-XX
  std::chrono::time_point<std::chrono::system_clock> file_timestamp; // = EmuEnv::GetDumpedDeleteFileTimestamp(); // = std::chrono::system_clock::now();
  ...
  FileDescriptor(uint64_t number, uint32_t path_id, uint64_t _file_size, 
      ...
      : FileDescriptor(number, path_id, _file_size, _smallest_seqno, _largest_seqno, std::chrono::system_clock::now(), false) {}

  FileDescriptor(uint64_t number, uint32_t path_id, uint64_t _file_size,
                 SequenceNumber _smallest_seqno, SequenceNumber _largest_seqno, 
                 std::chrono::time_point<std::chrono::system_clock> _file_timestamp, bool _expired_ttl)
      : table_reader(nullptr),
        ...
        expired_ttl(_expired_ttl) {EmuEnv::PopulatingVector(number);}
  // !END
  ...
  FileDescriptor& operator=(const FileDescriptor& fd) {
    ...
    // !YBS-aug17-XX
    file_timestamp = fd.file_timestamp;
    expired_ttl = fd.expired_ttl;
    // file_timestamp = std::chrono::system_clock::now();
    // std::cout << "DBG: " << std::chrono::system_clock::to_time_t(file_timestamp) 
    //           << " of file " << fd.packed_number_and_path_id << std::endl;
    // !END
    return *this;
  }
  ...
  std::chrono::time_point<std::chrono::system_clock> GetFileTimestamp() const {
    return file_timestamp; // & kFileNumberMask;
  }
  // !END
...
struct FileMetaData {
  ...
  FileMetaData(uint64_t file, uint32_t file_path_id, uint64_t file_size,
               ...
               const SequenceNumber& largest_seq, 
               std::chrono::time_point<std::chrono::system_clock> file_timestamp, // = EmuEnv::GetDumpedDeleteFileTimestamp(); // = std::chrono::system_clock::now();
               bool expired_ttl,
      ...
      : fd(file, file_path_id, file_size, smallest_seq, largest_seq, file_timestamp, expired_ttl),
...
class VersionEdit {
 public:
  ...
  void AddFile(int level, uint64_t file, uint32_t file_path_id,
               ...
               std::chrono::time_point<std::chrono::system_clock> file_timestamp, // !YBS-aug17-XX!
               bool expired_ttl, // !YBS-aug17-XX!
    ...
    new_files_.emplace_back(
        level, FileMetaData(file, file_path_id, file_size, smallest, largest,
                            ...
                            file_timestamp, expired_ttl, // !YBS-aug17-XX!
